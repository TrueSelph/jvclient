...
def render(
    ...
    response = call_action_walker_exec(agent_id, module_root, "list_documents", params)
    ...
    if st.button("Import", key=f"{model_key}_btn_import_knodes"):
        ...
        if data_to_import:
            if call_action_walker_exec(
                ...
                module_root,
                "import_knodes",
                {"data": data_to_import, "with_embeddings": with_embeddings},
            ):
    ...
    if not st.session_state[purge_key]:
        ...
        with col1:
            if st.button(
                ...
                if call_action_walker_exec(agent_id, module_root, "delete_collection"):
...
st.session_state.ROOT_ID = "000000000000000000000000"
st.session_state.TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY4NGJjMDA2Y2FmNWUzMjZlMmUwZDA2NyIsImVtYWlsIjoiYXJjLWFpQHRydWVzZWxwaC5jb20iLCJyb290X2lkIjoiMDAwMDAwMDAwMDAwMDAwMDAwMDAwMDAwIiwiaXNfYWN0aXZhdGVkIjp0cnVlLCJpc19hZG1pbiI6dHJ1ZSwiZXhwaXJhdGlvbiI6MTc1MDE1MTMxNCwic3RhdGUiOiI2d0pCV2cyZiJ9.-S6nlZBVEzSKNP5wk9oZX5DrhE0nC-HFpRVZ_bmuwno"
st.session_state.EXPIRATION = 1750151314


if __name__ == "__main__":
    router = StreamlitRouter()
    os.environ["JIVAS_BASE_URL"] = "https://arc-ai-jivas.trueselph.com"

    agent_id = "n:Agent:684bc1216d6cef8c8445f469"
    action_id = "n:TypesenseVectorStoreAction:684bc1956d6cef8c8445f4b3"

    info = json.loads(
        '{"name":"jivas/typesense_vector_store_action","author":"V75 Inc.","architype":"TypesenseVectorStoreAction","version":"0.0.3","meta":{"title":"Typesense Vector Store Action","description":"Integrates with typesense vector database for retrieval augmented generation tasks","group":"core","type":"vector_store_action"},"config":{"singleton":true,"path":"actions/jivas/typesense_vector_store_action","app":true,"module_root":"actions.jivas.typesense_vector_store_action","module":"actions.jivas.typesense_vector_store_action.typesense_vector_store_action","namespace":"jivas","package_name":"typesense_vector_store_action"},"dependencies":{"jivas":">=2.0.0-alpha.51","pip":{"typesense":">=0.21.0","langchain-core":">=0.3.47"}}}'
    )
